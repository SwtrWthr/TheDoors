...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? 'Добавить страну' : 'Редактировать страну'
    },
    countries() {
  ...
  methods: {
    async add_country() {
      ...
      await axios.post('/api/countries', this.editedItem)
        .then(res => {
    ...
    async edit_country(id) {
      await axios.put('/api/countries/' + id, {
    ...
    editItem(item) {
      console.log(this.countries.indexOf(item))
    ...
    deleteItemConfirm() {
      this.delete_country(this.deleteItemID)
      this.$store.dispatch('GET_COUNTRIES_API')
      this.closeDelete()
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        this.edit_country(this.editItemID)
      } else {
        this.add_country()
      }
      this.close()
